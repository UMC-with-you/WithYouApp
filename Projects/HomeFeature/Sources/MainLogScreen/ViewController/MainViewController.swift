...
class MainViewController: BaseViewController {
    ...
    let emptyLogView = MainEmptyLogView()
    let mainLogView = MainLogView()
    
    let viewModel : MainLogViewModel
    
    public init(viewModel : MainLogViewModel){
...
//            .disposed(by: disposeBag)
        ...
        // 로그 탭 클릭 이벤트
        viewModel.eclipsePosition.subscribe { [weak self] event in
            self?.mainLogView.moveEclipse(event)
        }
        .disposed(by: disposeBag)
    ...
    private func setActions(){
        ...
        self.mainLogView.ing
            .rx
            .tapGesture()
            .when(.recognized)
            .subscribe { [weak self] event in
                self?.viewModel.moveEclipseToIng()
            }
            .disposed(by: disposeBag)
        
        self.mainLogView.upcoming
            .rx
            .tapGesture()
            .when(.recognized)
            .subscribe(){ [weak self] _ in
                self?.viewModel.moveEclipseToUpcoming()
            }
            .disposed(by: disposeBag)
    }
...
///  MARK : - Coordinator 패턴으로 Refactor 예정
