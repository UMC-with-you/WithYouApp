...
class ReplyService : BaseService {
    ...
    //대댓글 추가
    func addReply(commentId : Int, content : String,_ completion : @escaping (Reply) -> Void){
    ...
    //대댓글 삭제
    func deleteReply(replyId: Int, _ completion : @escaping (ReplyIDResponse) -> Void){
        requestReturnsData(ReplyIDResponse.self, router: ReplyRouter.deleteReply(replyId: replyId), completion: completion)
    }
    
    //대댓글 수정
    func editReply(replyId : Int, content : String,_ completion : @escaping (ReplyIDResponse) -> Void){
