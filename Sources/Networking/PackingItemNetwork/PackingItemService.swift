...
class PackingItemService : BaseService {
    ...
    //짐 목록 조회
    func getItemList(travelId : Int, _ completion : @escaping (Any) -> Void){
        requestReturnsData(PackingItem.self, router: PackingItemRouter.getItemList(travelId: travelId), completion: completion)
    }
    
    //짐 추가
    func addItem(travelId : Int,itemName : String, _ completion : @escaping (Any) -> Void){
    ...
    //짐 삭제 -> travelId 응답
    func deleteItem(packingItemId : Int, _ completion : @escaping (Any) -> Void){
    ...
    //짐 패킹 토글
    func checkItem(packingItemId : Int, _ completion : @escaping (Any) -> Void){
    ...
    //짐 담당 멤버 설정
    func setItemMember(packingItemId : Int, memberId : Int,_ completion : @escaping (Any) -> Void ){
